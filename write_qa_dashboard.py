...
export default function QaDashboard() {
  ...
  const handleFail = async (bugId) => {
    const notes = window.prompt("Why did this fail?");
    if (notes === null) return;
    if (!notes.trim()) {
      alert("Notes are required for failure.");
      return;
    }
    try {
      ...
      alert("Failed to mark fail");
    }
  };

  const handleReassign = async (bugId) => {
    const notes = window.prompt("Reason for reassignment?");
    if (notes === null) return;
    if (!notes.trim()) {
      alert("A comment is required when reassigning a bug.");
      return;
    }
    // We expect the original developer's ID. Wait, assigning to same dev?
    // We need the bug's assigned_to id.
  ...
  const handleCreateBugListSuccess = () => {
    ...
    // Ideally refetch bug lists here if they were displayed, but currently we just refetch bugs.
    refetch(); 
  };
